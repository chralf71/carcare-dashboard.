...
Retrieval limits are fixed server-side constants, not new environment variables: at most 10 pages per job query at size 100 and 64 data requests per report plus one shared authentication request. Reaching a cap before traversal completes makes the entire report unavailable, including when the data-request cap is reached during employee lookup. No partial totals are returned. Existing 8-second request timeouts and 45-second per-client budgets apply; Current Tech Board directly queries `/jobs` with `shop` and `repairOrderStatusId` separately for statuses 1, 2, and 3, with concurrency 2. There are no per-RO job requests. Eligibility and incomplete labor are validated and filtered locally. Completed history and Current Tech Board have separate requests, clients, budgets, and UI states; either can succeed when the other fails. Technician requests are independent of shop/advisor requests, so an expensive scan cannot fail the existing report. Large shops may see an unavailable completed report until a documented completion filter or bounded historical synchronization is added.
...
Technician failures identify the affected history scan, current status query, or employee request budget. Page-limit errors explicitly state the 10-page/100-row cap and that more pages remain. Other request/validation failures are not labeled as confirmed limit failures. The frontend preserves these sanitized reasons on HTTP 503. The previous generic Preview error cannot identify which upstream operation actually failed. No limits were increased.
