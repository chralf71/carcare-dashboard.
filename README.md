...
- `GET /api/technician-summary?report=completed&date=YYYY-MM-DD` validates the Chicago date, then returns HTTP 200 with an unavailable report and no upstream calls, authentication, or employee lookup. The visible explanation is: “Unavailable — Tekmetric does not provide a verified job completed-date filter; scanning complete shop history is not safe for live reporting.” The completed calculation utility remains covered by synthetic tests but is not used for live historical retrieval.
- `GET /api/technician-summary?report=current` returns a current snapshot with `startedAt`, `asOf`, `historical: false`, and aggregate `technicianReport`. Historical date changes do not refresh this board.

Current Board calls `/jobs` with `shop` and `repairOrderStatusId: [1, 2, 3]` using the unchanged shared client's `URLSearchParams` encoding (`repairOrderStatusId=1%2C2%2C3`). It fully paginates one combined population, not three separate status populations or per-RO requests. The existing shop requests use the same helper with scalar statuses 5 and 6; this checkout had no separate array encoder. Combined jobs-filter acceptance still requires live confirmation.

Jobs must have `authorized=true`, `selected=true`, and `archived=false`, validated locally. Only labor with `complete=false` contributes incomplete assigned hours. Distinct eligible jobs containing incomplete labor contribute one active-job count. All pagination, duplicate/conflict validation, and reconciliation remain mandatory; partial totals are never returned. Limits remain 10 pages of 100 jobs and 64 data requests per board invocation, with existing request timeouts. Failures identify the combined statuses 1,2,3 population or employee request budget and remain isolated from shop/advisor reports.

Labor hours prefer a valid labor technician ID; an explicit null falls back to the job technician ID. Job counts use job ownership. Missing assignments are Unassigned, IDs absent from a complete directory are Unknown technician, and malformed/conflicting assignments use the invalid/conflict bucket. Duplicate jobs/labor deduplicate; conflicting quantities invalidate the report. Names join employee IDs only. No production names or IDs are configured. Only minimal employee identity and aggregate hours/counts are returned. No clocked time, efficiency, gross profit, or duplicate remaining-hours metric is calculated.

Advisor employee retrieval now starts before financial retrieval with its own client deadline and runs concurrently to avoid adding both deadlines; its complete result is reused by the advisor calculation. Financial retrieval gets a fresh deadline. Technician requests cannot consume either budget, and historical requests consume no upstream capacity. Separate Vercel invocations do not share an in-memory employee cache. Directory failures still preserve valid totals and explicitly mark names unavailable; restoring names in the live environment must be confirmed after Preview builds. Advisor calculations are unchanged.

The frontend preserves independent loading, failure, and stale-response handling. Board unavailable responses contain null totals and a sanitized reason, never upstream records/errors. Diagnostics remain disabled by default; Vercel layout/configuration and environment variables are unchanged.
